...
public class CollisionBox {
     ...
     * @param xDif The amount the object is going to change by in the next update in the x dir.
     * @param yDif The amount the object is going to change by in the next update in the y dir.
    ...
    public boolean intersects(CollisionBox collision, float xDif, float yDif) {
        ...
        if (c1R >= c2L && c1R <= c2R || c1L >= c2L && c1L <= c2R) {  // the rect overlap horizontally
            float c1T = this.origin.y + this.position.y;
            float c1B = this.origin.y + this.size.height + this.position.y;
            float c2T = collision.origin.y + collision.position.y + yDif;
            float c2B = collision.origin.y + this.size.height + collision.position.y + yDif;
            return c1B >= c2T && c1B <= c2B || c1T >= c2T && c1T <= c2B; // the rect overlap vertically
        } else {
            return false;
        }
    }
